...
describe('computeWidgetMood', () => {
  ...
  it('Last chance — within the end-of-day cutoff window', () => {
    ...
    expect(r.taskLine).toContain('Streak ends in');
  });

  it('Streak lost — takes priority over everything else', () => {
    const r = computeWidgetMood(baseInput({ streakLostToday: true }));
    expect(r.mood).toBe('sad');
    expect(r.taskLine).toBe('Start a new streak today');
  });
  ...
  it('Freeze saved — one-day flag set, no other higher-priority state', () => {
    const r = computeWidgetMood(baseInput({ freezeSavedToday: true }));
    expect(r.mood).toBe('freeze');
    expect(r.taskLine).toBe('A freeze covered yesterday');
  });

  it('Milestone — one-day flag set', () => {
    const r = computeWidgetMood(
      baseInput({
        milestoneToday: true,
        profile: { streakCount: 7, lastActiveDate: '2026-08-14' },
      }),
    );
    expect(r.mood).toBe('happy');
    expect(r.taskLine).toBe('7-day streak!');
  });

  it('Completed takes priority over Milestone/Freeze for the day', () => {
    const r = computeWidgetMood(
      baseInput({ dueRoutineCompletedToday: true, milestoneToday: true }),
    );
    expect(r.mood).toBe('completed');
  });
});
